...
async function loadData(){
  ...
  APP = { manifest, parts, playGuide, battlefieldGuide, movementGuide, keywordGuide, searchIndex: buildSearchIndex(parts) };
}
...
function renderSystemFoundation(card, accent='#93aac7'){
  return `<section class="sheet-card manual-foundation" style="--accent:${accent}"><div class="sheet-head compact"><div><div class="section-kicker">${esc(card.ref)}</div><div class="sheet-title" style="font-size:16px">${esc(card.title)}</div><div class="sheet-sub">${esc(card.summary)}</div></div><div class="head-tags"><button class="link-btn" data-open-target="${card.target}" style="min-height:34px;padding:6px 8px">Open</button></div></div><div class="manual-bands">${(card.bullets||[]).map((b,i)=>`<div class="manual-band band-${(i%2)+1}">× ${esc(b)}</div>`).join('')}</div></section>`;
}
function renderSystemManual(section){
  return `<section class="sheet-card manual-phase ${section.phaseClass||'system-battle'}" style="--accent:#93aac7"><div class="sheet-head"><div><div class="section-kicker">${esc(section.ref)}</div><div class="sheet-title">${esc(section.title)}</div><div class="sheet-sub">${esc(section.purpose)}</div></div><div class="head-tags"><button class="link-btn" data-open-target="${section.target}" style="min-height:34px;padding:6px 8px">Open Section</button></div></div><div class="phase-summary"><div class="phase-action-row">${(section.actions||[]).map(a=>`<span class="tag system">${esc(a)}</span>`).join('')}</div>${(section.notes||[]).length?`<div class="manual-bands compact">${section.notes.map((n,i)=>`<div class="manual-band band-${(i%2)+1}">× ${esc(n)}</div>`).join('')}</div>`:''}</div><div class="proc-stack">${(section.procedures||[]).map(proc=>renderProcedureCard(proc, 'system')).join('')}</div></section>`;
}
function renderKeywordGroup(group){
  return `<section class="sheet-card keyword-group" style="--accent:#b48ea1"><div class="sheet-head compact"><div><div class="section-kicker">Keyword family</div><div class="sheet-title" style="font-size:16px">${esc(group.title)}</div><div class="sheet-sub">${esc(group.summary)}</div></div></div><div class="keyword-chip-grid">${(group.keywords||[]).map(([label,target])=>`<button class="keyword-chip" data-open-target="${target}">${esc(label)}</button>`).join('')}</div></section>`;
}
function renderMovementManual(){
  const guide = APP.movementGuide;
  const part4 = findPart('part-4');
  return `<section class="sheet-card" style="--accent:#93aac7"><div class="sheet-head"><div><div class="section-kicker">Movement manual</div><div class="sheet-title">Part IV, rebuilt for table use</div><div class="sheet-sub">${esc(guide.intro.subtitle)}</div></div></div><div class="phase-strip phase-spine-manual battlefield-spine"><button class="phase-card system" data-open-target="part-4::4-1-measuring-distances"><strong>Measure</strong><span>Distance and elevation</span></button><button class="phase-card system" data-open-target="part-4::4-2-within-vs-wholly-within"><strong>Within</strong><span>Area checks</span></button><button class="phase-card system" data-open-target="part-4::4-3-the-leading-model"><strong>Leading Model</strong><span>Anchor the move</span></button><button class="phase-card system" data-open-target="part-4::4-4-unit-coherency"><strong>Coherency</strong><span>Rebuild the unit</span></button><button class="phase-card system" data-open-target="part-4::4-6-gap-clearance-and-model-size"><strong>Gap Clearance</strong><span>Path legality</span></button></div></section><div class="manual-stack">${guide.foundations.map(c=>renderSystemFoundation(c, '#93aac7')).join('')}${guide.systems.map(renderSystemManual).join('')}</div><section class="sheet-card" style="--accent:#93aac7"><div class="sheet-head compact"><div><div class="section-kicker">Full compendium text</div><div class="sheet-title" style="font-size:16px">Exact Part IV rules</div><div class="sheet-sub">Use the movement manual above to answer the procedural shape first. Open the full entries below when you need precise wording, measurements, or pathing edge cases.</div></div></div></section><div class="part-stack">${part4 ? renderPart(part4) : ''}</div>`;
}

function renderBattlefieldManual(){
  const guide = APP.battlefieldGuide;
  const part7 = findPart('part-7');
  return `<section class="sheet-card" style="--accent:#93aac7"><div class="sheet-head"><div><div class="section-kicker">Battlefield manual</div><div class="sheet-title">Part VII, rebuilt for rulings</div><div class="sheet-sub">${esc(guide.intro.subtitle)}</div></div></div><div class="phase-strip phase-spine-manual battlefield-spine"><button class="phase-card system" data-open-target="part-7::7-1-line-of-sight"><strong>LoS</strong><span>Visibility and trace</span></button><button class="phase-card system" data-open-target="part-7::7-1-line-of-sight::7-1-1-cover"><strong>Cover</strong><span>Full and direct cover</span></button><button class="phase-card system" data-open-target="part-7::7-2-engagement-and-engagement-range"><strong>Engagement</strong><span>Who is tied up</span></button><button class="phase-card system" data-open-target="part-7::7-3-markers-and-tokens"><strong>Markers & Tokens</strong><span>Objects on the field</span></button></div></section><div class="manual-stack">${guide.foundations.map(c=>renderSystemFoundation(c)).join('')}${guide.systems.map(renderSystemManual).join('')}</div><section class="sheet-card" style="--accent:#93aac7"><div class="sheet-head compact"><div><div class="section-kicker">Full compendium text</div><div class="sheet-title" style="font-size:16px">Exact Part VII rules</div><div class="sheet-sub">Use the battlefield manual above to answer the ruling shape first. Open the full entries below when you need precise wording or less common edge cases.</div></div></div></section><div class="part-stack">${part7 ? renderPart(part7) : ''}</div>`;
}
function renderKeywordReference(){
  const guide = APP.keywordGuide;
  const part11 = findPart('part-11');
  return `<section class="sheet-card" style="--accent:#b48ea1"><div class="sheet-head"><div><div class="section-kicker">Play glossary</div><div class="sheet-title">Part XI, rebuilt for lookup</div><div class="sheet-sub">${esc(guide.intro.subtitle)}</div></div></div><div class="link-grid keyword-toplinks"><button class="link-btn" data-jump="part-11::place">PLACE</button><button class="link-btn" data-jump="part-11::instant">INSTANT</button><button class="link-btn" data-jump="part-11::engaged">ENGAGED</button><button class="link-btn" data-jump="part-11::line-of-sight-los">LINE OF SIGHT</button></div></section><div class="manual-stack keyword-groups">${guide.groups.map(renderKeywordGroup).join('')}</div><section class="sheet-card" style="--accent:#b48ea1"><div class="sheet-head compact"><div><div class="section-kicker">Full glossary</div><div class="sheet-title" style="font-size:16px">Alphabetical Part XI</div><div class="sheet-sub">Use the grouped sheets above for function-first lookup. Use the full glossary below for exact wording and less common terms.</div></div></div></section><div class="part-stack">${part11 ? renderPart(part11) : ''}</div>`;
}
...
function renderSystems(){ const filters=[{key:'all',label:'All Systems',active:state.systemFilter==='all'},{key:'movement',label:'Movement',active:state.systemFilter==='movement'},{key:'core',label:'Core',active:state.systemFilter==='core'},{key:'battle',label:'Battlefield',active:state.systemFilter==='battle'},{key:'advanced',label:'Advanced',active:state.systemFilter==='advanced'}]; let parts=APP.parts.filter(p=>p.group==='systems'); if(state.systemFilter==='movement'){ SYSTEMS.innerHTML = renderMovementManual(); return; } if(state.systemFilter==='core') parts=parts.filter(p=>['part-2','part-3','part-4','part-5','part-6'].includes(p.id)); if(state.systemFilter==='battle'){ SYSTEMS.innerHTML = renderBattlefieldManual(); return; } if(state.systemFilter==='advanced') parts=parts.filter(p=>['part-10'].includes(p.id)); renderCollection(SYSTEMS, parts, {label:'Rules Systems', subtitle:'Definitions, measurements, terrain, abilities, and governing logic', filters, filterView:'systems'}); }
function renderReference(){ REFERENCE.innerHTML=`<section class="sheet-card" style="--accent:#96a6b8"><div class="sheet-head"><div><div class="section-kicker">Reference</div><div class="sheet-title">Fast answers during play</div><div class="sheet-sub">Use quick reference for operating flow, grouped keywords for fast term lookup, and bookmarks for repeated table rulings.</div></div></div><div class="link-grid"><button class="link-btn" data-jump="part-12">Quick reference</button><button class="link-btn" data-view-go="systems">Systems view</button><button class="link-btn" data-jump="part-12::12-2-round-sequence">Round sequence</button><button class="link-btn" data-jump="part-12::12-4-phase-2-assault">Assault reference</button></div></section><section class="sheet-card" style="--accent:#b4889b"><div class="sheet-head compact"><div><div class="section-kicker">Bookmarks</div><div class="sheet-title" style="font-size:16px">Saved rulings</div></div></div><div class="saved-list">${state.bookmarks.length?state.bookmarks.map(b=>`<div class="mini-row"><div class="mini-main"><div class="mini-title">${esc(b.label)}</div><div class="mini-sub">Saved for reuse</div></div><div class="head-tags"><button class="link-btn" data-open-target="${b.target}" style="min-height:34px;padding:6px 8px">Open</button><button class="link-btn" data-remove-bookmark="${b.target}" style="min-height:34px;padding:6px 8px">Remove</button></div></div>`).join(''):'<div class="mini-row"><div class="mini-main"><div class="mini-title">No bookmarks yet</div><div class="mini-sub">Save a Part or specific rule with ★.</div></div></div>'}</div></section><section class="sheet-card" style="--accent:#96a6b8"><div class="sheet-head compact"><div><div class="section-kicker">Printable</div><div class="sheet-title" style="font-size:16px">Print field manual</div></div></div><div class="link-grid one"><button class="link-btn" id="referencePrintBtn">Open printable version</button></div></section>${renderKeywordReference()}<div class="part-stack">${APP.parts.filter(p=>p.group==='reference' && p.id!=='part-11').map(renderPart).join('')}</div>`; }
